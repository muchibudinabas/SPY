...
<div class="form-group">
  ...
  <div class="col-sm-9">
    <select class="form-control" id="ID_PERIODE" name="NAMA"  value="" required>
    <option value="" selected="selected" >Periode Wisuda</option>
      @foreach($pw as $pww)
      <option value="{{$pww->ID_PERIODE_WISUDA}}">{{indonesiaDate($pww->TGL_WISUDA)}}</option>
      @endforeach
    </select>
  </div>
</div>
