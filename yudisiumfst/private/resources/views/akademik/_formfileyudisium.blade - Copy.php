...
<div class="form-group">
  ...
  <div class="col-sm-9">
    <select class="form-control" id="ID_JADWAL_YUDISIUM" name="NAMA"  value="" required disabled="">
    <!-- <option value="" selected="selected" >Tanggal Yudisium</option> -->
      @foreach($jy as $jyy)
      <option value="{{$jyy->ID_JADWAL_YUDISIUM}}">{{indonesiaDate($jyy->TGL_YUDISIUM)}}</option>
      @endforeach
    </select>
  </div>
</div>
