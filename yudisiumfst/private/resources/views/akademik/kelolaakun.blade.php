                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    @foreach ($data as $persons)
                      <tr id="person{{ $persons->NIP }}">
                        ...
                        @if ($persons->AKTIVASI == 1)
                        <td ><span class="label label-success">Aktif</span></td>
                        @endif
                        @if ($persons->AKTIVASI == 0)
                        <td ><span class="label label-danger">Non Aktif</span></td>
                        @endif
                        

                        <td style="text-align:center;width:15%;">
                        <button class="btn btn-xs bg-blue open-modal" value="{{$persons->NIP}}"><i class="glyphicon glyphicon-edit"></i> Kelola</button>
        ...
        <div id="myModal" class="modal fade" role="dialog" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true">
          <div class="modal-dialog">
            <div class="modal-content">
                ...
                <h4 class="modal-title" id="myModalLabel">Kelola Akun</h4>
                </div>
                <div class="modal-body">
                {!! Form::open(array('id' => 'frm', 'name' => 'frm', 'class' => 'form-horizontal')) !!}
                <input id="token" type="hidden" value="{{$encrypted_token}}">
                @include('akademik._formkelola')  
                {!! Form::close() !!}
    ...
    $(".save").click(function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                {
                    var person = '<tr id="person' + data.data.NIP + '"><td>' + data.data.NIP + '</td><td>' + data.data.NAMA_PEGAWAI + '</td><td>' + data.data.JABATAN + '</td><td>' + data.data.UNIT + '</td><td>' + data.data.USERNAME + '</td><td><span class="label label-danger">' + data.data.AKTIVASI0 + '</span><span class="label label-success">' + data.data.AKTIVASI1 + '</span></td>';
                    person += '<td style="text-align:center;width:15%;"><button class="btn btn-xs bg-blue open-modal" value="' + data.id + '"><i class="glyphicon glyphicon-edit"></i> Kelola</button></td></tr>';
