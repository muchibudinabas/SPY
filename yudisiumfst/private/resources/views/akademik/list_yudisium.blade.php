                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    @foreach ($person as $persons)
                      <tr id="person{{ $persons->NIM }}">
                        ...
                        <td >Perempuan</td>
                        @endif
                        <td>{{ indonesiaSortDate($persons->TGL_LULUS) }}</td>
                        <td>{{ $persons->UNIT }}</td>
    ...
    $(".save").click(function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                {
                    var person = '<tr id="person' + data.data.NIM + '"><td>' + data.data.NIM + '</td><td><a id="nama-' + data.data.NIM + '" href="{{ url('detail_mhs') }}/' + data.data.NIM + '">' + data.data.NAMA + '</td><td>' + data.data.JENIS_KELAMIN + '</td><td>' + data.data.TGL_LULUS + '</td><td>' + data.data.UNIT + '</td><td>' + data.data.NO_IJAZAH + '</td><td>' + data.data.IPK + '</td><td>' + data.data.SKS + '</td><td>' + data.data.ELPT + '</td><td>' + data.data.SKP + '</td>';
                    person += '<td style="text-align:center;width:15%;"><button class="btn btn-xs bg-blue open-modal" value="' + data.data.NIM + '"><i class="glyphicon glyphicon-edit"></i> Input No Ijazah</button>';
                    person += '&nbsp;<button class="btn btn-xs bg-green oke" value="' + data.data.NIM + '"><i class="glyphicon glyphicon-ok"></i> Oke</button></td></tr>';
                    
                    if (state == "add"){ 
                        $('#person-list').append(person);
                    }else{ 
                        // $("#person" + NIM).replaceWith(person);
                        $("#person" + data.data.NIM).replaceWith(person);
                    }
